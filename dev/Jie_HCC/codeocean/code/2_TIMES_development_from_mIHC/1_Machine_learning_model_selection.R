...
##read in data
library(readxl)
load("/data/Input_2_TIMES_development_from_mIHC/XGBoost_TIMES_Performance.RData")
...
for (trial_i in 1:100){
  ...
  #training
  linearMod <- glm("Relp_y ~ NKatBorder_x_VIM + NKatTumor_x_VIM",data = train_df_1,family=binomial,control=list(maxit=100)) #linear model
  model_SVM_R <- svm(Relp_y ~ NKatBorder_x_VIM + NKatTumor_x_VIM,data=train_df_1, kernel = "radial",probability = TRUE,decision.values = TRUE) #support vector machines - radial
  model_SVM_L <- svm(Relp_y ~ NKatBorder_x_VIM + NKatTumor_x_VIM,data=train_df_1, kernel = "linear",probability = TRUE,decision.values = TRUE) #support vector machines - linear
  model_RF <- randomForest(Relp_y ~ NKatBorder_x_VIM + NKatTumor_x_VIM,data = train_df_1,ntree =60,importance=TRUE,proximity=TRUE,replace=F) #random forest
  model_DT <- rpart("Relp_y ~ NKatBorder_x_VIM + NKatTumor_x_VIM",method="class",data = train_df_1,maxdepth=10) #decision tree
  gbm_tunepara <- data.frame(n.trees=1000,interaction.depth=10,shrinkage=0.01,n.minobsinnode=3) 
  set.seed(trial_i)
  fn_x <- "as.factor(Relp_y) ~ NKatBorder_x_VIM + NKatTumor_x_VIM"
  ...
  ppv_train_vec <- c(ppv_train_vec, ppv(tb)) #PPV: posotive predictive value
...
#write.csv(predLiver_relapse_absoluteX,"VIM_performace_liver_all_25_NEW_FINAL.csv")
