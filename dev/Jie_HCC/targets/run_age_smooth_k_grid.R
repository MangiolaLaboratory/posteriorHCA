#!/usr/bin/env Rscript
# Initialise / refresh the age-smooth k-grid targets pipeline and run it.
#
...
#   NAMES=fit_SPON2_k3  Rscript ... # tar_make selected names only
...
script_path <- file.path(case_dir, "targets", "age_smooth_k_grid_tar_script.R")
root_path <- "/hpcfs/groups/phoenix-hpc-mangiola_laboratory/chen/HPC_posterior/age_smooth_k_grid"
...
cli_alert_info(
  if (is.null(tar_names)) {
    "Starting tar_make() for the full pipeline (12 fits: 4 genes × k=3,4,5) ..."
  } else {
...
cli_alert_success(glue("Done. Fits / CSVs under {out_dir}"))
